...
select
    idLobbyGame + (select max(idLobbyGame)
    from tb_lobby_stats_player) as idLobbyGame,
    idPlayer,
    idRoom,
    qtKill,
    qtAssist,
    ...
    qtHitRightLeg,
    flWinner,
    datetime((select max(dtCreatedAt)
    from tb_lobby_stats_player), '+1 day') as dtCreatedAt
